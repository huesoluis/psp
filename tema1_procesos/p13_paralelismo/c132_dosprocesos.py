...
# Función para generar dos listas a partir de un archivo de números
def generar_listas(fichero):
    """
    Lee un archivo de números y divide la lista en dos partes iguales.

    Parámetros:
    fichero (str): Ruta del archivo de texto con una lista de números enteros.

    Retorna:
    tuple: Dos listas de números enteros.
    """
    with open(fichero, 'r') as f:
        numeros = list(map(int, f.readlines()))
    mitad = len(numeros) // 2
    lista1 = numeros[:mitad]
    lista2 = numeros[mitad:]
...
def main():
    ...
    num_procesos = 2  # Número de procesos utilizados

    # Genera las dos listas dividiendo el archivo en dos partes
    lista1, lista2 = generar_listas(fichero)
